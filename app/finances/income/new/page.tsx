...
export default function NewIncome() {
  ...
  const save = async () => {
    ...
    setSaving(true)
    const { error } = await supabase.from('income').insert({ amount: parseFloat(amount), description, category, date })
  ...
  return (
    <div style={{ maxWidth: 500 }}>
      <div style={{ marginBottom: 40 }}>
        <h1 style={{ fontSize: 32, fontWeight: 700, color: '#fff', letterSpacing: '-0.6px' }}>Log Income</h1>
        <p style={{ fontSize: 14, color: '#5a5a5a', marginTop: 8 }}>Record income received.</p>
      </div>
      <div style={{ background: '#1e1e22', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: 28, boxShadow: '0 2px 20px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>{label('Description')}<input style={inputStyle} value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Payment from Acme Corp" /></div>
        <div>{label('Category')}
